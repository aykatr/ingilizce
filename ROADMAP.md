...
**Ek: görsel revizyon (kullanıcı referans tasarımına göre)** — İş mantığı/veri/GameEngine'e dokunulmadan, yalnızca `#screen-menu` markup+CSS'i kullanıcının paylaştığı referans görsele göre yeniden tasarlandı: mor ışın/bulut zeminli panel, "ANA SAYFA"/"SES AÇIK" ikon-etiket butonları (yeni `#btn-home-menu` — Başlangıç Ekranı'na döner, salt gezinme, veri etkilenmez), kartlar için döngüsel pastel renk paleti (8 tema, kart id'sine göre değil sırasına göre atanır), her kartta 3 yıldız + "TAMAMLANDI"/"BAŞLAMADIN" rozeti (referans tasarımda bazı kartlarda kısmi 1/3-2/3 yıldız durumu var, ama veri modelimizde yalnızca tamamlandı/tamamlanmadı ikili durumu olduğu için **kısmi ilerleme uydurulmadı** — tamamlanan kart 3/3 dolu yıldız, tamamlanmamış 0/3 gösterir), alt kısımda koyu mor istatistik çubuğu (Toplam Puan/Tamamlanan Kart/Rozetler + yeşil ilerleme çubuğu yüzdesiyle). Tamamlanma durumu tamamen `.is-completed` CSS sınıfı üzerinden (yıldız/rozet/onay işareti hepsi CSS descendant seçicileriyle) sürüyor — `game-ui.js`'in zaten yaptığı `classList.add('is-completed')` dışında yeni bir DOM güncelleme mantığı eklenmedi. Playwright ile görsel doğrulama (390px/360px/420px genişlik, yatay taşma yok) ve mevcut 15+26 kontrol tekrar çalıştırılarak regresyon olmadığı doğrulandı.

## Faz 9 — Optimizasyon

Faz başlamadan önce kapsam güncellendi: önce Version 1.0 öncesi bir **UI/kalite kontrol turu** (referans tasarımlarla birebir görsel uyum) yapıldı, ardından asıl Faz 9 (performans/güvenlik/production hazırlığı) başlayacak. Tur 5 aşamalı işletildi: Aşama 1 (UI Denetimi, kod yazmadan rapor) → onay → Aşama 2 (referansa uyum) → Aşama 4 (responsive) → Aşama 5 (test) → **Aşama 3 (UX iyileştirmeleri, ayrı onayla)** — hepsi tamamlandı.
...
### Aşama 3 — UX İyileştirmeleri ✅ (onaylı kısıtlarla uygulandı)

- [x] **Hover efektleri** — yalnızca `@media (min-width: 768px)` içinde (tablet+desktop, aynı breakpoint 1024px+'i de kapsadığı için tek tanım yeterli oldu), mobilde hiç yok. Butonlar (`.btn-pill-primary`, `.btn-nav`), ikon butonlar (`.icon-btn`, `.menu-icon-circle`), Kart Seçim Menüsü kartları (`.menu-card`), cevap kartları (`.option-card`), QR sekmeleri/butonu — hepsine hafif `translateY`/`scale` + `box-shadow` derinleşmesi eklendi. Yalnızca `transform`/`box-shadow`/`background-color` (compositor-dostu, layout tetiklemez), abartılı animasyon yok.
- [x] **Basma efektleri** — mevcut `:active` davranışları (scale/translateY değerleri) **değiştirilmedi**, yalnızca eksik olan yerlere eklendi (`.btn-nav`'ın hiç `:active` durumu yoktu, `.menu-icon-circle`'ın hiç geçişi yoktu) ve tüm butonlar/kartlar tek bir ortak token çiftinden (`--transition-press: 0.1s ease`, `--transition-hover: 0.18s ease`) geçiş süresi/easing alacak şekilde birleştirildi — önceden 0.08s/0.12s/0.15s karışık değerler vardı.
- [x] **Loading geri bildirimi** — yalnızca gerçekten ağ isteği bekleyen 3 aksiyonda: kart seçimi, cevap gönderme, Tekrar Oyna. Saf CSS `.is-loading` sınıfı (dönen spinner, `transform: rotate` + `opacity`, GPU-dostu) — JS tarafında yalnızca `classList.add/remove`, işlem bitince (başarı ya da hata fark etmeden `finally` bloğunda) otomatik kalkıyor. Cevap gönderirken tıklanmayan diğer 3 seçenek de `pointer-events: none` ile geçici kilitleniyor (çifte-tıklama/yarış durumunu önler). "Ana Menü dönüşü" **kasıtlı olarak spinner almadı** — bu aksiyon hiç ağ isteği yapmıyor (salt ekran geçişi), sıfır gecikmeli bir işleme spinner eklemek gereksiz DOM işlemi olurdu (performans kuralına aykırı).
- [x] **GSAP geçişleri gözden geçirildi** — mevcut süreler (0.08s-0.5s mikro-etkileşimler, ~1.1-1.6s okunabilir toast'lar) zaten "akıcı/kısa/çocuk dostu" aralıkta bulundu, **değiştirilmedi** (gereksiz değişiklik yapılmadı).
- [x] **Genel tutarlılık** — tüm interaktif elemanlar aynı iki token'ı (`--transition-press`, `--transition-hover`) paylaşıyor; `prefers-reduced-motion: reduce` için spinner animasyonu duraklatılıp statik bir halka olarak bırakıldı (ek erişilebilirlik önlemi, ayrıca istenmedi ama düşük maliyetli).
- [x] **Performans** — eklenen tüm efektler `transform`/`opacity`/`box-shadow`/`background-color` ile sınırlı tutuldu (layout/reflow tetiklemez), yeni network isteği eklenmedi, DOM güncellemeleri tek `classList` çağrısına indirildi. Lighthouse resmi olarak çalıştırılmadı (projede kalıcı bir bağımlılık değil) — istenirse ek olarak çalıştırılabilir.

**Teslim (Aşama 3):** Playwright ile 9 yeni kontrol (`test-stage3-ux.js` — desktop hover transform doğrulama, mobil'de hover uygulanmadığının viewport doğrulaması, 3 aksiyonun hepsinde loading spinner'ın istek sürerken göründüğü + bitince kalktığı, cevap sırasında diğer seçeneklerin kilitlendiği — ağ isteği `page.route()` ile bilerek geciktirilerek gözlemlendi) — **9/9 geçti**. Mevcut 15 (`test-card-menu.js`) + 11 (`test-stage2-ui.js`) + 26 (`test-audio.js`) regresyon paketleri tekrar çalıştırıldı, hepsi temiz.
