...
- **Başlangıç ekranı özelleştirme**: `App\Services\StartScreenService` + `/admin/settings/start-screen` — tüm görseller (`start_*_image` anahtarları, `settings` tablosunda) ve üç metin alanı (`start_title/description/button_text`) admin panelden yönetilir. Kod içinde **sabit görsel yolu veya metin kullanılmaz**; görsel ayarlanmamışsa ilgili `<img>` hiç render edilmez (kırık görsel yerine zarif gizleme — `<?php if (!empty($startScreen['x'])): ?>` deseni). Yeni bir dekoratif görsel alanı eklerken bu deseni tekrar kullanın (`StartScreenService::IMAGE_FIELDS`'e ekleyin, view'da conditional render).
