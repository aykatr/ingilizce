...
Mevcut örnek: `AuthService` (login/logout/şifre değiştirme), `LicenseService` (lisans oluşturma/durum/aktivasyon takibi), `SettingService` (site URL, varsayılan süre/puan gibi tekil ayarlar), `CategoryService` (kategori CRUD + silme koruması), `QuestionService` (soru+4 seçenek+medya birlikte yönetimi).
